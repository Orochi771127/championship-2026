...
| Toolbar icons are authored, not original | 2 / 2 / 4 | 16 | Open, and deliberately not shipped. `ui/UI_Icon_training` yields the original hand, food, capsule, clean, wound, medicine and manage shapes, and a 4x baseline is built at `docs/art/production/toolbar/faithful-hd4x`. Its own palette bank 0 renders them orange and brown, while the original plays them silver and lights the selected slot gold (video t030), so the runtime binds a palette this pack does not carry. Promoting bank 0 would look less like the original than the authored icons do. Needs the overlay palette binding traced first. |
