...
- Championship: the board is reachable from the battle menu, as ui/conference_list_item.nxr is in the original. A round is fought as an ordinary battle against the team its own pool drew, and the verdict the economy settles on is what writes the flag; the run survives Save/Continue, and the prize and title result wait for the whole run as 0210D0C8's gate does. The tournament descriptor carries rounds, prize and two more fields and no entry fee, so a round is entered free — battle_menu/titlematch_top_sub_scene.nxr prices a title match, and that fee now shows against what the player holds on the schedule board. Open: the round banner ui/battle_title_champion_main.nxr shows is not drawn, ui/conference_list_item.nxr does not identify its cup — traced, and the scene turns out to carry layout only: even its base node declares a cell that is a glyph rather than the plate it draws, so the sprites are bound by the code that opens it ([receipt](research/CONFERENCE_LIST_ITEM_CUP_BINDING_2026-09-11.json)) — so the icon is a stand-in, and whether the original's own save carries a run is untraced — see [the receipt](research/CHAMPIONSHIP_RUN_PERSISTENCE_2026-09-11.json).
