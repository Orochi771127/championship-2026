"""Build the six remaining fixed-window OPM cage cell banks."""
...
WORK=ROOT/'docs/art/production/original-character-cage-r1/cage-base3d-v1';OUT=WORK/'opm-remaining-fixed-v1'
...
def main():
    ...
    for fid in selected:
        ...
        for cell_id in cells(contract):
            ...
            manifest['status']='ART_PROPOSAL_OFFLINE_CELL_BANK_ONLY';manifest['sourceCellId']=cell_id
            manifest['consumer']='Offline per-sequence frame selection only; not runtime animation'
        ...
        assert_unchanged(ROOT,contract);print('REMAINING_FIXED_CELL_BANK_EXPORTED',fid,len(cells(contract)))
    print('REMAINING_FIXED_BATCH_EXPORTED_CLOCK_UNRESOLVED')
