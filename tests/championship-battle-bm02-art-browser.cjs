...
const BASE_URL = process.env.CHAMPIONSHIP_BATTLE_ART_QA_URL || "http://127.0.0.1:8732/battle-art-review.html?arena=bm02";
const CHROME = process.env.CHAMPIONSHIP_CHROME || "C:/Program Files/Google/Chrome/Application/chrome.exe";
const OUTPUT = require("./browser-qa-output.cjs")("art/battle/bm02-r1");
const VIEWPORTS = [
...
(async () => {
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      assert.equal(diagnostics.runtime.review.fieldId, "field_bm02_01");
      ...
      const screenshot = `battle-bm02-${viewport.width}x${viewport.height}.png`;
