...
test("M201 all-83 audit locks 47 unique visuals and complete Sub-to-Main reuse", () => {
  ...
  assert.equal(all83Reuse.technicalCandidateUniqueCount, 9);
  assert.equal(all83Reuse.pendingUniqueCount, 38);
...
test("M201 hybrid review atlas exposes all 83 stable texture keys without promotion", () => {
  ...
  assert.equal(reviewManifest.technicalCandidateUniqueCount, 9);
  assert.equal(reviewManifest.pendingUniqueCount, 38);
