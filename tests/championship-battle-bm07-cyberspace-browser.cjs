...
(async () => {
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      await page.goto(`${BASE_URL}?arena=bm07`, { waitUntil: "networkidle" });
      ...
      assert.equal(diagnostics.runtime.review.fieldId, "field_bm07_01");
      assert.equal(diagnostics.runtime.review.layerCount, 1);
      ...
      const screenshot = `battle-bm07-${viewport.width}x${viewport.height}.png`;
      await page.screenshot({ path: path.join(OUTPUT, screenshot), fullPage: true });
      results.push({ arena: "bm07", fieldId: "field_bm07_01", viewport, screenshot, diagnostics, problems });
      await context.close();
    }
  } finally {
    await browser.close();
